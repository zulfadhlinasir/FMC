...
#User input required - FMC Hostname,Username,password
...
try:
    ...
    # r = requests.post(auth_url, headers=headers, auth=requests.auth.HTTPBasicAuth(username,password), verify=r"C:\Users\mzbnasi\Desktop\SSL Cert\FMCRootCe.cer")
...
try:
    ...
    # r = requests.get(url, headers=headers, verify=r'C:\Users\mzbnasi\Desktop\SSL Cert\FMCRootCe.cer')
    status_code = r.status_code
    resp = r.text
    if (status_code == 200):
        print("GET successful. Printing the FTD information for managed by", fmcName.upper())
        data=r.json()
        print(r.json)
        DeviceNameCombine=[]
        DeviceIDCombine=[]
